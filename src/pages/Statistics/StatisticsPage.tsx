...
function ImpressionsTab() {
    ...
    const handleSearch = (v: string) => {
        ...
        searchTimer.current = setTimeout(() => { setPage(0); setSearch(v); }, 400);
    };

    const handleExport = async () => {
        setExporting(true);
        try {
            const rows = await adminService.getImpressionsExport({ search: search || undefined });
            downloadCSV(rows, `impressions_${new Date().toISOString().slice(0,10)}.csv`);
    ...
    return (
        <div>
            <div style={{ display: "flex", gap: 12, marginBottom: 18, flexWrap: "wrap" }}>
                <div style={{ flex: 1, minWidth: 200, position: "relative" }}>
                    ...
                    <input
                        ...
                        style={{ width: "100%", paddingLeft: 34, paddingRight: 12, height: 38, border: "1px solid var(--border)", borderRadius: 8, background: "var(--bg-input,#1a1a2e)", color: "var(--text-main)", fontSize: 13, boxSizing: "border-box" }}
                    />
                </div>
                <button
                    onClick={load}
                    style={{ display: "flex", alignItems: "center", gap: 6, padding: "0 14px", height: 38, border: "1px solid var(--border)", borderRadius: 8, background: "transparent", color: "var(--text-main)", cursor: "pointer", fontSize: 13 }}
                >
                ...
                <button
                    ...
                    disabled={exporting}
                    style={{ display: "flex", alignItems: "center", gap: 6, padding: "0 14px", height: 38, border: "1px solid rgba(16,185,129,0.4)", borderRadius: 8, background: "rgba(16,185,129,0.1)", color: "#10b981", cursor: "pointer", fontSize: 13, opacity: exporting ? 0.6 : 1 }}
                >
            ...
            <div style={{ fontSize: 13, color: "var(--text-muted)", marginBottom: 10 }}>
                Jami: <b style={{ color: "var(--text-main)" }}>{total.toLocaleString()}</b> ta taassurot
            </div>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                    ...
                    <tbody>
                        {loading ? (
                            ...
                            >
                                <td style={{ padding: "10px 12px", color: "var(--text-muted)" }}>{page * PAGE_SIZE + i + 1}</td>
                                <td style={{ padding: "10px 12px" }}>{row.username ? `@${row.username}` : "—"}</td>
                                <td style={{ padding: "10px 12px" }}>{row.firstName || "—"}</td>
            ...
            {totalPages > 1 && (
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 16, justifyContent: "flex-end" }}>
                    <button onClick={() => setPage(p => Math.max(0, p - 1))} disabled={page === 0}
                        style={{ width: 32, height: 32, border: "1px solid var(--border)", borderRadius: 6, background: "transparent", color: "var(--text-main)", cursor: page === 0 ? "not-allowed" : "pointer", opacity: page === 0 ? 0.4 : 1 }}>
                        <ChevronLeft size={16} />
                    </button>
                    <span style={{ fontSize: 13, color: "var(--text-muted)" }}>{page + 1} / {totalPages}</span>
                    <button onClick={() => setPage(p => Math.min(totalPages - 1, p + 1))} disabled={page >= totalPages - 1}
                        style={{ width: 32, height: 32, border: "1px solid var(--border)", borderRadius: 6, background: "transparent", color: "var(--text-main)", cursor: page >= totalPages - 1 ? "not-allowed" : "pointer", opacity: page >= totalPages - 1 ? 0.4 : 1 }}>
...
function ClicksTab() {
    ...
    const handleExport = async () => {
        setExporting(true);
        try {
            const rows = await adminService.getClicksExport({ search: search || undefined });
    ...
    return (
        <div>
            ...
            <div style={{ fontSize: 13, color: "var(--text-muted)", marginBottom: 10 }}>
                Jami: <b style={{ color: "var(--text-main)" }}>{total.toLocaleString()}</b> ta klik
            </div>
...
function BotUsersTab() {
    ...
    useEffect(() => {
        setBotsLoading(true);
        adminService.getAllBots({ limit: 100 })
    ...
    const handleExport = async () => {
        ...
        setExporting(true);
        try {
            const rows = await adminService.getBotUsersExport(selectedBot, { activeDays, search: search || undefined });
            downloadCSV(rows, `bot_users_${new Date().toISOString().slice(0,10)}.csv`);
        } catch { alert("Export xatoligi"); }
        setExporting(false);
    };
        ...
        <div>
            <div style={{ display: "flex", gap: 12, marginBottom: 18, flexWrap: "wrap" }}>
                <div style={{ minWidth: 220 }}>
                    <select
                        value={selectedBot}
                        onChange={e => { setSelectedBot(e.target.value); setPage(0); setSearch(""); setSearchInput(""); }}
                        style={{ width: "100%", height: 38, border: "1px solid var(--border)", borderRadius: 8, background: "var(--bg-input,#1a1a2e)", color: "var(--text-main)", fontSize: 13, padding: "0 12px" }}
                    >
                        <option value="">{botsLoading ? "Yuklanmoqda..." : "Bot tanlang..."}</option>
                        {bots.map((b: any) => (
                ...
                <div>
                    <select
                        value={activeDays}
                        onChange={e => { setActiveDays(Number(e.target.value)); setPage(0); }}
                        style={{ height: 38, border: "1px solid var(--border)", borderRadius: 8, background: "var(--bg-input,#1a1a2e)", color: "var(--text-main)", fontSize: 13, padding: "0 12px" }}
                    >
            ...
            {!selectedBot ? (
                ...
                <>
                    <div style={{ fontSize: 13, color: "var(--text-muted)", marginBottom: 10 }}>
                        Jami: <b style={{ color: "var(--text-main)" }}>{total.toLocaleString()}</b> ta faol foydalanuvchi (oxirgi {activeDays} kun)
                    </div>
...
function BroadcastsTab() {
    ...
    const load = async () => {
        setLoading(true);
        try {
            const res = await adminService.getAllBroadcasts({ limit: PAGE_SIZE, offset: page * PAGE_SIZE });
    ...
    return (
        <div>
            ...
            <div style={{ fontSize: 13, color: "var(--text-muted)", marginBottom: 10 }}>
                Jami: <b style={{ color: "var(--text-main)" }}>{total.toLocaleString()}</b> ta broadcast
            </div>
            ...
            <div style={{ overflowX: "auto" }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                    <thead>
                        <tr style={{ borderBottom: "1px solid var(--border)" }}>
                            {["#", "Advertiser", "Bot", "Holat", "Maqsad", "Yuborildi", "Xatolik", "Narx", "Sana"].map(h => (
                                <th key={h} style={{ padding: "10px 12px", textAlign: "left", color: "var(--text-muted)", fontWeight: 600, whiteSpace: "nowrap" }}>{h}</th>
                            ))}
                        </tr>
                    </thead>
                    <tbody>
                        {loading ? (
                            ...
                            <tr><td colSpan={9} style={{ textAlign: "center", padding: 40, color: "var(--text-muted)" }}>Broadcast topilmadi</td></tr>
                        ) : data.map((row, i) => (
                            <tr key={row.id} style={{ borderBottom: "1px solid var(--border)" }}
                                onMouseEnter={e => (e.currentTarget.style.background = "rgba(255,255,255,0.03)")}
                                onMouseLeave={e => (e.currentTarget.style.background = "transparent")}
                            >
                                ...
                                <td style={{ padding: "10px 12px" }}>
                                    {row.advertiser ? (
                                        <span>{row.advertiser.firstName || ""} {row.advertiser.lastName || ""}{row.advertiser.username ? ` (@${row.advertiser.username})` : ""}</span>
                                    ) : "—"}
                                </td>
                                <td style={{ padding: "10px 12px" }}>{row.bot?.username ? `@${row.bot.username}` : "—"}</td>
                                <td style={{ padding: "10px 12px" }}><BroadcastStatusBadge status={row.status} /></td>
                                <td style={{ padding: "10px 12px", textAlign: "center" }}>{(row.targetCount ?? 0).toLocaleString()}</td>
                                <td style={{ padding: "10px 12px", textAlign: "center", color: "#10b981" }}>{(row.sentCount ?? 0).toLocaleString()}</td>
                                <td style={{ padding: "10px 12px", textAlign: "center", color: "#ef4444" }}>{(row.failedCount ?? 0).toLocaleString()}</td>
