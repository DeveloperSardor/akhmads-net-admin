...
const empty = {
    slug: "", nameUz: "", nameRu: "", nameEn: "", icon: "📌", sortOrder: 0, isActive: true,
};
...
export function CategoriesPage() {
    ...
    const toggleActive = async (cat: Category) => {
        try {
            await apiClient.put(`/admin/categories/${cat.id}`, { isActive: !cat.isActive });
            fetchCategories();
        } catch (e) {
            console.error(e);
        }
        ...
        <div className="animate-in">
            ...
            <div className="page-head">
                <div className="page-head-left">
                    ...
                    <div className="section-sub">Bot va reklama kategoriyalarini boshqarish — qo'shish, tahrirlash, o'chirish</div>
                </div>
            ...
            <div className="stats-grid" style={{ marginBottom: 24 }}>
                ...
                <div className="stat-card">
                    <div className="stat-icon-wrap" style={{ background: "rgba(239, 68, 68, 0.12)", color: "#ef4444" }}>
                        <XCircle size={20} />
                    </div>
                    <div className="stat-info">
                            ...
                            {loading && (
                                <tr>
                                    <td colSpan={8} style={{ textAlign: "center", padding: 60 }}>
                                        ...
                                        <div style={{ marginTop: 12, color: "var(--text3)", fontSize: 13 }}>Yuklanmoqda...</div>
                                    </td>
                                </tr>
                            )}
                                ...
                                <tr>
                                    <td colSpan={8}>
                                        <div className="empty-state">
                                            ...
                                            <div className="empty-text">Yangi kategoriya qo'shish uchun yuqoridagi tugmani bosing</div>
                                        </div>
                                    </td>
                                </tr>
                            )}
                            {filtered.map((cat) => (
                                <tr key={cat.id} className="hover-row" style={{ opacity: cat.isActive ? 1 : 0.5 }}>
                                    <td>
                                        <div style={{ display: "flex", alignItems: "center", gap: 6, color: "var(--text3)" }}>
                                            <GripVertical size={14} strokeWidth={1.5} />
                                            <span className="mono" style={{ fontSize: 12 }}>{cat.sortOrder}</span>
                                        </div>
                                    </td>
                                    <td>
                                        <div style={{
                                    ...
                                    <td>
                                        <span className="mono" style={{ fontSize: 12, color: "var(--accent2)", background: "rgba(139, 92, 246, 0.08)", padding: "3px 8px", borderRadius: 6 }}>
                                            {cat.slug}
                                        </span>
                                    </td>
                                    <td style={{ fontWeight: 500, fontSize: 13 }}>{cat.nameUz}</td>
                                    <td style={{ fontWeight: 500, fontSize: 13 }}>{cat.nameRu}</td>
                                    <td style={{ fontWeight: 500, fontSize: 13 }}>{cat.nameEn}</td>
                ...
                <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setModal(null)}>
                    <div className="modal" style={{ maxWidth: 520 }}>
                        <div className="modal-title">
                            {modal.mode === "add" ? "➕ Yangi kategoriya" : "✏️ Kategoriyani tahrirlash"}
                        </div>
                        ...
                        <div style={{
                            display: "grid", gap: 14,
                            padding: 16, borderRadius: 12,
                            background: "var(--surface1)", border: "1px solid var(--border-light)",
                            marginBottom: 16
                        }}>
                            ...
                            <div className="modal-field" style={{ marginBottom: 0 }}>
                                ...
                                <input className="modal-input" value={form.nameUz}
                                    onChange={e => setForm({ ...form, nameUz: e.target.value })}
                                    placeholder="Musiqa" />
                            </div>
                            <div className="modal-field" style={{ marginBottom: 0 }}>
                                <span className="modal-label">🇷🇺 Ruscha</span>
                                <input className="modal-input" value={form.nameRu}
                                    onChange={e => setForm({ ...form, nameRu: e.target.value })}
                                    placeholder="Музыка" />
                            </div>
                            <div className="modal-field" style={{ marginBottom: 0 }}>
                                <span className="modal-label">🇬🇧 Inglizcha</span>
                                <input className="modal-input" value={form.nameEn}
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                            <div className="modal-field">
                                ...
                                <input className="modal-input mono" type="number" value={form.sortOrder}
                                    onChange={e => setForm({ ...form, sortOrder: parseInt(e.target.value) || 0 })} />
                            </div>
                            <div style={{ display: "flex", alignItems: "end", paddingBottom: 10 }}>
                                <label style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer", fontSize: 14, color: "var(--text-main)" }}>
                                    <div
                                    ...
                                    <span style={{ fontWeight: 500 }}>{form.isActive ? "Faol" : "Nofaol"}</span>
                                </label>
                            </div>
                        </div>
                        ...
                        <div className="modal-actions">
                            <button
                                ...
                                onClick={save}
                                disabled={saving || !form.slug || !form.nameUz || !form.nameRu || !form.nameEn}
                            >
                                {saving ? "Saqlanmoqda..." : modal.mode === "add" ? "✓ Qo'shish" : "✓ Saqlash"}
                            </button>
