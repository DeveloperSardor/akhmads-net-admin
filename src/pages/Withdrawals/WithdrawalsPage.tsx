...
export function WithdrawalsPage({
  ...
  const { data: response, isLoading } = useAdminWithdrawals(
  ...
  const pendingTotal = withdrawals
    ...
    .reduce((s: number, w: any) => s + parseFloat(w.netAmount || 0), 0);
  ...
  return (
    <div>
      ...
      >
        {["all", "REQUESTED", "PENDING_REVIEW", "COMPLETED", "REJECTED"].map(
          (s) => (
            <button
              key={s}
            ...
            <tbody>
              ...
              {!isLoading && withdrawals.length === 0 && (
                <tr>
                  <td colSpan={8}>
                    <div className="empty">
                      <div className="empty-icon">○</div>Hech narsa topilmadi
                    </div>
                  </td>
                </tr>
              )}
              {withdrawals.map((w: any) => (
                <tr key={w.id}>
                  <td>
                    ...
                    >
                      ...
                      <span className="mono" style={{ fontSize: 12 }}>
                        @{w.user?.username || w.user?.firstName || "noma'lum"}
                      </span>
                    </div>
                  </td>
                  ...
                  <td>
                    <span className={`badge ${wdStatusColor[w.status]}`}>
                      {wdStatusMap[w.status] || w.status}
                    </span>
                  </td>
                    ...
                    w.status === "PENDING_REVIEW" ? (
                      <div className="action-btns">
                        <button
                          ...
                          onClick={() =>
                            setModal({ type: "approve-withdrawal", data: w })
                          }
                        >
                          ✓ Tasdiqlash
                        </button>
                        ...
                        <button
                          ...
                          title="Ko'rish"
                          onClick={() =>
                            setModal({ type: "view-withdrawal", data: w })
                          }
                        >
                          <Eye size={15} />
                        </button>
