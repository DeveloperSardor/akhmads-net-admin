...
export function AdminsPage({ showToast }: any) {
  ...
  const addAdmin = () => {
    if (!newUsername.trim()) {
      showToast("Username yoki Telegram ID kiritish shart!", "error");
      return;
    }
    ...
    if (!targetUser) {
      showToast(
        "Bunday foydalanuvchi topilmadi. Avval tizimga kirgan bo'lishi kerak.",
        "error",
      );
      return;
    }
    updateRole.mutate(
      { id: targetUser.id, role: newRole },
      {
        onSuccess: () => {
          ...
          setShowForm(false);
          showToast(
            `@${targetUser.username || targetUser.firstName} — ${ROLE_META[newRole]?.label || newRole} sifatida tayinlandi ✓`,
          );
        },
  ...
  const removeAdmin = (user: UserProfile) => {
    if (user.role === "SUPER_ADMIN" && role !== "superadmin") {
      showToast("Super adminni o'chirish uchun sizda huquq yo'q!", "error");
      return;
    }
    if (
      !confirm(
        `@${user.username || user.firstName} dan admin huquqlarini olasizmi?`,
      )
    )
      return;
    updateRole.mutate(
      { id: user.id, role: "ADVERTISER" },
      {
  ...
  return (
    <div className="elite-analytics-wrap">
      ...
      {showForm && (
        <div
          className="elite-card"
          style={{ marginBottom: 24, border: "1px solid rgba(167,139,250,.3)" }}
        >
          <div
            style={{
              display: "flex",
            ...
            <div>
              ...
              <div
                style={{
                  display: "grid",
                  gridTemplateColumns:
                    role === "superadmin" ? "1fr 1fr" : "1fr 1fr 1fr",
                  gap: 8,
                }}
              >
                {(role === "superadmin"
          ...
          >
            {newRole === "SUPER_ADMIN" &&
              "👑 Barcha huquqlar: foydalanuvchilar, adminlar, to'lovlar, analitika, sozlamalar"}
            {newRole === "ADMIN" &&
              "🛡️ Moderatsiya, foydalanuvchilar, to'lovlar, analitika"}
            {newRole === "MODERATOR" && "🔍 Reklamalar va botlar moderatsiyasi"}
            {newRole === "SUPPORT" &&
              "💬 Foydalanuvchilarga yordam, moderatsiya navbati"}
          </div>
      ...
      <div className="card" style={{ padding: 0, overflow: "hidden" }}>
        ...
        >
          ...
          <div
            style={{ position: "relative", flex: "1 1 260px", maxWidth: 300 }}
          >
            <Search
              size={14}
              style={{
